...
class StrideImage:
    ...
    def __sub__(self, other):
        red = [x - y for x, y in zip(self.pixels_red, other.pixels_red)]
        green = [x - y for x, y in zip(self.pixels_green, other.pixels_green)]
        blue = [x - y for x, y in zip(self.pixels_blue, other.pixels_blue)]
        alpha = [x - y for x, y in zip(self.pixels_alpha, other.pixels_alpha)]
        pixels_zip = zip(red, green, blue, alpha)
        pixels = [Pixel(r, g, b, a) for r, g, b, a in pixels_zip]

        return StrideImage(self.resolution, pixels)
